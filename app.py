...
composition_dict = {
    ...
    "三分割法・黄金比・白銀比（安定と美しさの比率）": "A strictly structured composition adhering to the Rule of Thirds or Golden Ratio, with key elements placed precisely on intersection lines and focal points.",
    ...
    "額縁構図（外枠で囲って中央を際立たせる）": "CRITICAL RULE: A visible, distinct decorative frame or border surrounding the entire composition, emphasizing the central content. The frame style must match the chosen genre and art style (e.g., whimsical branches, a simple clean line, or an ornate gold border).", # 🚨 ここが超重要！AIにフレームを描くのを義務付け！
...
# Zの法則・Fの法則をデフォルトにする
composition_ui = st.selectbox("神・構図リスト（目的で選んでね！）", comp_options, index=comp_options.index("Zの法則・Fの法則（視線誘導の王道）"))

# Zの法則・Fの法則を見つけられなかった場合のフォールバック
if not composition_ui:
    composition_ui = "Zの法則・Fの法則（視線誘導の王道）"

# Zの法則・Fの法則が選択肢にあるか確認、なければデフォルト
if composition_ui not in composition_dict:
    composition_ui = "Zの法則・Fの法則（視線誘導の王道）"

# 🚨 ここが超重要！日本語をAIへの強力な英語指示に変換！
composition_instruction = composition_dict.get(composition_ui, composition_ui)
...
if st.button("🪄 読者の心を動かす図解プロンプトを生成する"):
    ...
    if text_strictness == "🚫 指定した文字だけを厳格に入れる（勝手な追加NG）":
        text_rule = "Do not add any extra text or subtitles. Use ONLY the exact title and details provided. Respect line breaks."
    else:
        text_rule = "Feel free to add highly relevant, short subtitles or catchphrases to enhance the design."
    ...
    if text_background == "✨ おまかせ（AIが一番読みやすいデザインを自動で選ぶ！）":
        bg_instruction = "Auto-select the best readable text box styles (e.g., speech bubbles, clean panels)."
    elif text_background == "🏷️ タイトルのみ強調（見出しだけリボンやバナー風にする）":
        bg_instruction = "Apply a stylish background (like a banner or ribbon) ONLY to the title. Keep detail texts clean."
    ...
    data_for_gemini = {
        ...
        "design_concept": {
            ...
            "composition_structure": composition_instruction, # 🚨 日本語から強力な英語指示に変換！これで発動する！
