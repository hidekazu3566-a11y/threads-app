...
# --- UIとAIへの英語指示の変換辞書 ---
composition_dict = {
"王道のZの法則（視線誘導 × 多キャラ感情同期 × 密度強化）": (
        "ELITE Z-PATTERN LAYOUT WITH MULTIPLE EMOTIONAL GUIDES & HIGH DENSITY: "
        
        # 1. 【厳格なZ字の視線誘導と分散配置】中央に1匹ではなく、各所にキャラを置く
        "1. STRICT Z-PATH WITH DISTRIBUTED GUIDES: Arrange visual elements along a clear 'Z' trajectory (Top-L -> Top-R -> Bottom-L -> Bottom-R). "
        "Instead of one central subject, place a small version of the character next to EACH individual text container as a visual guide. "

        # 2. 【強弱と視認性の絶対死守】文字はパキッと大きく！
        "2. CLEAR VISUAL HIERARCHY: The 'TITLE' MUST be massive, bold, and heavy. "
        "Each detail box MUST contain clearly legible text with medium size and medium font weight. Visibility is 100% required for mobile screens. "

        # 3. 【個別ボックスごとの感情同期】ここが最大の進化ポイント！
        "3. INDIVIDUAL BOX EMOTION SYNC: Each character guide MUST have an expression and pose that 100% reflects the specific content of the ADJACENT text box. "
        "Analyze each box individually: If a box is about a 'struggle', that specific character must look pensive or sad. If it's a 'success', that specific character must smile brightly. Use a variety of poses (pointing, sleeping, thinking) to match the context. "

        # 4. 【文脈を読んだ豪華な装飾】ボックスの中身に合わせてアイコンを添える
        "4. ORNATE CONTEXTUAL SHAPES & PROPS: Surround each box with 'secondary props' that act as visual metaphors for the text (e.g., if a box mentions 'rest', add a small bed or moon icon). "
        "Fill the whitespace elegantly with theme-appropriate accents to create a rich, professional atmosphere. "

        # 5. 【背景の密度と空気感】
        "5. ATMOSPHERIC BACKGROUND DENSITY: Add subtle, faded thematic textures or artistic silhouettes to the background to eliminate empty space and enhance the worldview. "
    ),
