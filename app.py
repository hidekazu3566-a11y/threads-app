...
# --- UIとAIへの英語指示の変換辞書 ---
composition_dict = {
"王道のZの法則（視線誘導 × 多キャラ感情同期 × 洗練・引き算版）": (
        "ELITE Z-PATTERN LAYOUT WITH NUANCED EMOTIONAL GUIDES & ELEGANT DENSITY: "
        ...
        # 3. 【控えめで洗練された装飾（ここを引き算！）】
        # アイコンを「大量」に出すのではなく、「厳選して1〜2個」添える指示。
        "3. REFINED CONTEXTUAL ACCENTS: Render each block in a detailed container, but keep 'secondary props' SUBTLE and MINIMAL. "
        "Instead of many small icons, choose ONE high-quality visual metaphor per box. Ensure decorations do not clutter or distract from the character and text. "
        ...
        # 5. 【余白を活かす背景密度（ここを調整！）】
        # 背景を「埋める」のではなく、空間を「感じさせる」程度にする。
        "5. ELEGANT BACKGROUND AIR: Add very faint, faded thematic textures to the background. Prioritize 'elegant whitespace' to make the main content pop. Avoid adding too many floating objects. "
    ),
