...
# --- UIとAIへの英語指示の変換辞書 ---
composition_dict = {
 "王道のZの法則（視線誘導 × 密度強化 × 視認性重視）": (
        "ELITE Z-PATTERN LAYOUT WITH HIGH READABILITY, RICH ACCENTS & CONTEXTUAL FLOW: "
        
        # 1. 【厳格な視線誘導】左上→右上→左下→右下の「Z」の軌道に要素を配置する
        "1. STRICT Z-PATH FLOW: Arrange all visual elements (Title, Character, and Detail Boxes) along a clear 'Z' trajectory. "
        "Start at the Top-Left, move to the Top-Right, diagonal to the Bottom-Left, and end at the Bottom-Right. This path must be intuitive and unobstructed. "

        # 2. 【強弱と視認性の両立】本文を「スマホで絶対読める太さとサイズ」に固定！
        "2. CLEAR VISUAL HIERARCHY: The 'TITLE' MUST be massive, bold, and heavy. "
        "In contrast, all text inside the decorative containers MUST be clearly legible and sufficiently large (medium size), using a medium font weight to ensure 100% visibility even on small screens. Readability is the absolute highest priority. "

        # 3. 【文脈を読んだ装飾とあしらい】AIが内容を分析して、空間を埋める「小道具」を散りばめる
        "3. ORNATE CONTEXTUAL SHAPES & ACCENTS: Render each text block inside a highly detailed decorative container. "
        "Analyze the text context and surround the boxes and character with theme-appropriate 'visual accents' and 'secondary props' (e.g., if the theme is 'growth', add vines and sparkling light). "
        "These decorations must fill the whitespace elegantly to create a professional, high-density look. "

        # 4. 【文脈同期型・感情ミラーリング】タイトルの空気感に、猫の表情を完璧に合わせる
        "4. NUANCED EMOTIONAL SYNCHRONIZATION (TITLE-ONLY): The character's expression MUST reflect the specific intensity and vibe of the 'TITLE'. "
        "IF positive, choose an appropriate smile (gentle to bright). IF negative or serious, ZERO TOLERANCE for smiles—use pensive or focused expressions. "
        "Ignore detail keywords; only the Title dictates the character's mood. "

        # 5. 【背景の密度】ただの塗りつぶしにせず、テクスチャやシルエットを入れる
        "5. ATMOSPHERIC BACKGROUND DENSITY: Add subtle, faded thematic textures or artistic silhouettes to the background to increase the overall visual richness and eliminate empty 'template' feelings. "
    ),
