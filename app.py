...
# --- UIとAIへの英語指示の変換辞書 ---
composition_dict = {
"王道のZの法則（視線誘導 × 多キャラ感情同期 × 形状コントラスト）": (
        "ELITE Z-PATTERN LAYOUT WITH DYNAMIC SHAPE VARIETY & EMOTIONAL GUIDES: "
        
        # 1. 【Z字の視線誘導とキャラ配置】
        "1. STRICT Z-PATH WITH DISTRIBUTED GUIDES: Arrange elements along a clear 'Z' trajectory. Place a small character guide next to each text box. "

        # 2. 【強弱と視認性】タイトルは巨大、本文は読みやすいサイズと太さ。
        "2. CLEAR VISUAL HIERARCHY: Title must be massive and bold. Detail text must be medium size and weight for 100% legibility. "

        # 3. 【形状のコントラストと機能的デザイン（ここを強化！）】
        # 全部同じ形にせず、内容に合わせて「形」を変えてメリハリをつける。
        "3. DYNAMIC & FUNCTIONAL SHAPES: Avoid repetitive box shapes. Mix styles like speech bubbles, scrolls, and geometric panels to create visual rhythm. "
        "CRITICAL: If the text is a transition or bridge (e.g., 'For example', 'Next', or 'Steps'), the container itself MUST be a large, stylized directional arrow shape to physically guide the eye. "

        # 4. 【個別ボックスごとの感情同期】
        "4. INDIVIDUAL BOX EMOTION SYNC: Each character guide must have an expression/pose that 100% reflects the adjacent text box's context. "

        # 5. 【洗練された密度の引き算】
        # 装飾は「1箱に1つ」の厳選したアイコンで、背景は空気感を感じさせる程度に。
        "5. SOPHISTICATED MINIMALISM: Use only ONE high-quality visual metaphor per box to avoid clutter. "
        "Maintain 'elegant whitespace' and use very faint background textures to make the main content and its unique shapes stand out. "
    ),
